...
def put_pts_on_ellipse(xc,yc,rx,ry,n):
    t = np.linspace(0,math.pi*2,n+1)
    pts = np.zeros((n,2))
    pts[:,0] = xc+rx*np.cos(t[0:n])
    pts[:,1] = yc+ry*np.sin(t[0:n])
    return pts
...
def find_ABbdry_pts_ellipse(pts,xc,yc,rx,ry,h0):
    # suitable for the case when A and B are balls of radius r
    ind = np.argwhere(np.sqrt(((pts[:,0]-xc)**2)/rx**2+((pts[:,1]-yc)**2)/ry**2) - 1.0 < h0*1e-2)
    Nind = np.size(ind)
    ind = np.reshape(ind,(Nind,))
    return Nind,ind
...
def stima3_var_diffusion_matrix(verts,Mmatr):
    Aux = np.ones((3,3))
    Aux[1:3,:] = np.transpose(verts)
    rhs = np.zeros((3,2))
    rhs[1,0] = 1
...
def FEM_committor_solver_cylindrical_symmetry(pts,tri,Aind,Bind,fpot,rfac,beta):
    #rfac = r^{d-2}
    Npts = np.size(pts,axis=0)
    Ntri = np.size(tri,axis=0)
    Dir_bdry = np.hstack((Aind,Bind))
...
def FEM_committor_solver_var_diffusion_matrix(pts,tri,Aind,Bind,Fpts,M11pts,M12pts,M22pts,beta):
    Npts = np.size(pts,axis=0)
    Ntri = np.size(tri,axis=0)
    Dir_bdry = np.hstack((Aind,Bind))
    free_nodes = np.setdiff1d(np.arange(0,Npts,1),Dir_bdry,assume_unique=True)
    ...
    for j in range(Ntri):
        ...
        indt = np.array(ind)[:,None]
        A[indt,ind] = A[indt,ind] + stima3_var_diffusion_matrix(verts,Mmatr)*fac
...
def reactive_current_and_transition_rate_var_diffusion_matrix(pts,tri,Fpts,M11pts,M12pts,M22pts,beta,q,Z):
    Npts = np.size(pts,axis=0)
    Ntri = np.size(tri,axis=0)
    # find the reactive current and the transition rate
    Rcurrent = np.zeros((Ntri,2)) # reactive current at the centers of mesh triangles
...
# probability_reactive_Langevin(pts,tri,Hamiltonian,beta,q,qminus,Z)
...
def probability_last_A_Langevin(pts,tri,pts_Amesh,tri_Amesh,fpot,beta,qminus,Z):
    Npts = np.size(pts,axis=0)
    Ntri = np.size(tri,axis=0)
    Npts_Amesh = np.size(pts_Amesh,axis=0)
    Ntri_Amesh = np.size(tri_Amesh,axis=0)
...
def mean_force_var_diffusion_matrix(pts,tri,Fpts):
    Npts = np.size(pts,axis=0)
    Ntri = np.size(tri,axis=0)
    # find the reactive current and the transition rate
